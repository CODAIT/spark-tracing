...
object StatJVMStart extends StatSource {
  ...
  override def extract(events: RDD[EventTree], resolve: ServiceMap): Iterable[(Any, Double)] =
...
object StatJVMs extends StatSource {
  ...
  override def extract(events: RDD[EventTree], resolve: ServiceMap): Iterable[(Any, Double)] =
    events.filter(row => row(3)(0).is("SpanStart") && row(3)(2).is("JVMStart")).map(_ => () -> 0.0).collect
}

object StatExecs extends StatSource {
  val name: String = "Executors"
  override def extract(events: RDD[EventTree], resolve: ServiceMap): Iterable[(Any, Double)] =
    events.filter(StatUtils.isDagEvent(_, "ExecutorAdded")).map(_ => () -> 0.0).collect
}

object StatJobs extends StatSource {
  val name: String = "Jobs"
  override def extract(events: RDD[EventTree], resolve: ServiceMap): Iterable[(Any, Double)] =
    events.filter(StatUtils.isDagEvent(_, "JobSubmitted")).map(_ => () -> 0.0).collect
}

object StatTasks extends StatSource {
  val name: String = "Tasks"
  override def extract(events: RDD[EventTree], resolve: ServiceMap): Iterable[(Any, Double)] = StatTaskLength.extract(events, resolve)
}
...
object StatBlockUpdates extends StatSource {
  ...
  override def extract(events: RDD[EventTree], resolve: ServiceMap): Iterable[(Any, Double)] =
    events.filter(StatUtils.fnArgs(_, "org.apache.spark.storage.BlockManagerMaster.updateBlockInfo").isDefined)
      .map(_(2).get -> 0.0).collect
}

object ColCount extends StatCol {
  val name: String = "Count"
  val colType: ColType = Num
  override def calculate[T : Numeric](values: Iterable[(Any, T)]): Int = values.size
}

class ColPercentile(p: Int) extends StatCol {
  val name: String = p.toString + "%"
  val colType: ColType = Num
  private def percentile[T : Numeric](x: Iterable[T], p: Int): T = {
    val items = x.toSeq
    if (items.isEmpty) implicitly[Numeric[T]].fromInt(0)
    else items.sorted.apply((scala.math.ceil(p / 100.0 * x.size).toInt - 1).max(0))
  }
  override def calculate[T : Numeric](values: Iterable[(Any, T)]): T = percentile(values.map(_._2), p)
}

object ColArgMin extends StatCol {
  val name: String = "Min at"
  val colType: ColType = Str
  override def calculate[T : Numeric](values: Iterable[(Any, T)]): String =
    values.toSeq.sortBy(_._2).headOption.map(_._1.toString).getOrElse("")
}
...
object ColArgMax extends StatCol {
  ...
  val colType: ColType = Str
  override def calculate[T : Numeric](values: Iterable[(Any, T)]): String =
    values.toSeq.sortBy(_._2).lastOption.map(_._1.toString).getOrElse("")
}
