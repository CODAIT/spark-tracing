...
class Events(events: RDD[EventTree], resolve: ServiceMap, format: Formatter) extends OutputBlock {
  ...
  def data: Iterable[Seq[Any]] = events.filter(row => !row(3)(0).isAny(nonEvents)).flatMap(row =>
    resolve.mainService(row(1).get).map(service => Seq(row(2).get, service.id, format.format(row(3))))
  ).collect
}

class Spans(events: RDD[EventTree], resolve: ServiceMap, format: Formatter) extends OutputBlock {
  ...
  def data: Iterable[Seq[Any]] = {
    ...
    all.flatMap(span =>
      resolve.mainService(span._1._1).map(service => Seq(span._1._2, span._2, service.id, format.format(span._1._3)))
    ).collect
  }
}
